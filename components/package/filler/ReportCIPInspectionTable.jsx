// components/package/filler/ReportCIPInspectionTable.jsx
...
// ==================== Main Component: LINE A Table ====================
const ReportCIPInspectionTable = ({
  cipData,
  onSave,
  isEditable = true,
  ...
  const isTableComplete = () => {
    ...
    const copOk = copRecords.every((cop) => {
      const dur =
        cop.stepType === "COP"
          ? cop.time67Min
          : cop.stepType === "SOP"
          ...
          ? cop.time60Min
          : null;
      return dur && cop.tempActual && cop.startTime && cop.endTime;
    });
    return everyStepOk && copOk && kodeOperator && kodeTeknisi;
  };

  const isHeaderComplete = () => {
    const { processOrder, line, cipType, operator, posisi, flowRate } = cipData || {};
    const headerOk = processOrder && line && cipType && operator && posisi;
    const flowOk = line === "LINE A" ? !!flowRate : true;
  ...
  const updateStepField = (index, field, value) => {
    ...
    // AUTO-FILL kodeOperator/kodeTeknisi tetap manual—tidak diubah di sini
  };
  ...
  // =============== VALIDASI & PENGEMASAN DATA (SUBMIT/DRAFT) ===============
  const validateAndSave = (submitMode = false) => {
    if (submitMode) {
      ...
      const invalidTimeSteps = cipSteps.filter((step) => {
        ...
        return (step.startTime && !timeRegex.test(step.startTime)) || (step.endTime && !timeRegex.test(step.endTime));
      });
      ...
      const invalidCop = copRecords.filter((cop) => !cop.tempActual || !cop.startTime || !cop.endTime);
      ...
      const invalidCopTimes = copRecords.filter((cop) => {
        const timeRegex2 = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
        return (cop.startTime && !timeRegex2.test(cop.startTime)) || (cop.endTime && !timeRegex2.test(cop.endTime));
      });
    ...
    // MODE DRAFT: kirim state apa adanya agar tersimpan
    return {
      steps: cipSteps,
      copRecords,
      kodeOperator,
